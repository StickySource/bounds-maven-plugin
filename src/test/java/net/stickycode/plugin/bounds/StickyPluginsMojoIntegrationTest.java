...
public class StickyPluginsMojoIntegrationTest {

  @Test
  public void matchVersionRanges() {
    StickyPluginsMojo mojo = new StickyPluginsMojo();
    assertThat(mojo.matchVersion("1.0,2").matches()).isFalse();
    assertThat(mojo.matchVersion("[1.0,2]").matches()).isFalse();
    assertThat(mojo.matchVersion("[1.0,2)").matches()).isFalse();
    assertThat(mojo.matchVersion("[1.0,2.0)").matches()).isFalse();
    assertThat(mojo.matchVersion("[1.0,)").matches()).isFalse();
    assertThat(mojo.matchVersion("[1.0,2.3.4)").matches()).isFalse();
    assertThat(mojo.matchVersion("[1.0.4,2.3.4)").matches()).isFalse();
    assertThat(mojo.matchVersion("[1.0.4, 2.3.4)").matches()).isFalse();
    assertThat(mojo.matchVersion("[1.0.4 , 2.3.4)").matches()).isFalse();
    assertThat(mojo.matchVersion("[1.0.4 ,2.3.4)").matches()).isFalse();
    assertThat(mojo.matchVersion("[1.0.4-SNAPSHOT,2.3.4-SNAPSHOT)").matches()).isFalse();
  ...
  public void update()
    ...
    new StickyPluginsMojo().updateDependency(pom, artifact, "[3.6,4)");
  ...
  public void updateWithClassifier()
    ...
    new StickyPluginsMojo().updateDependency(pom, artifact, "[2.6,3)");
    XPathContext context = new XPathContext("mvn", "http://maven.apache.org/POM/4.0.0");
